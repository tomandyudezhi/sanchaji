...
@section('content')
  <div class="content-wrap">
    ...
    @if(!empty($tag_content))
        ...
        <span id="hidden_text-{{$v->articles->id}}" style="display:none;">{!!$v->articles->content!!}</span>
            <p id="note-{{$v->articles->id}}" style="overflow: hidden;text-overflow: ellipsis;display: -webkit-box;-webkit-line-clamp: 3;-webkit-box-orient: vertical;color:#999;"></p>
        </article>
  <script type="text/javascript">
    $(function(){
    $('#note-{{$v->articles->id}}').text($('#hidden_text-{{$v->articles->id}}').text());
  });
  </script>
      </article>
      @endforeach
